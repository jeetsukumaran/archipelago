...
\begin{description}
    ...
    \item[``model\_id'']  \hfill \\
        ...
        Each tree generated under this model will have the metadata item, ``\textit{model\_id}'', with this value associated with it.
        Each set of summary statistics calculated on each tree will have an additional field, ``\textit{model\_id}`` added to it to document the generating model by the value given to this field.
        The classification programn ``\textit{archipelago-classify.py}'', will expect the training data summary statistics to have a column with this name, and it will use the values of this column to determine the origin or source of the data.
    \item[``areas'']  \hfill \\
