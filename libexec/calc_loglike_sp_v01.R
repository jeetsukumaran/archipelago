	...
	for (i in edges_to_visit)
		...
		if (sparse==FALSE)
			...
			if (any(is.nan(condlikes_Right)))
				...
				cat("\n\nindependent likelihood %*% relative_probs_of_each_state_at_branch_top_AT_node_DOWNPASS returned NaNs:\n\n")
				cat("j=", j, "\n")
				cat("phy2$edge.length[i]=", phy2$edge.length[j], "\n")
			...
			if (hooknode_TF == TRUE)
				...
				for (ii in starting_1based_states_index:numstates)
					...
					if (null_range_allowed == TRUE)
						...
						# The Lstate_1based_indexes are based on ALL states MINUS the null range
