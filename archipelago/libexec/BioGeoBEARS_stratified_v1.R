	...
	# 							print(temporary_fixlikes)
					...
					for (uj in edges_to_visit_uppass)		# Since we are going backwards
						...
						if (hooknode_TF == TRUE)
							...
							# BUG BUG BUG FIX
							# 2014-01-30_NJM
							# added 2014-03-20 here -- NJM
							# These are, by default, ALL states, including null range
							if (null_range_allowed == TRUE)
								{
								starting_1based_states_index = 2
								} else {
								starting_1based_states_index = 1
								}

							
							# Here, we go through each ancestral state; then, conditional on an
							# ancestral state, we go through each allowed descendant pair and 
							# add up the probabilities
							for (ii in starting_1based_states_index:numstates)
								{
								...
								# BUG BUG BUG FIXED 2014-03-20_NJM
								# A certain number of rows in COO_weights_columnar[[1]] will match this ancstate
								# (be sure to convert R 1-based index to C++ 0-based index
								ancstate_matches_TF = (ii == (COO_weights_columnar[[1]] + 1 + 1))
								# (+1 for 0-based, +1 since null range excluded from speciation matrix
								#  in COO_weights_columnar)
								...
								if (null_range_allowed == TRUE)
									...
									# 0 on the UPPASS
									# 
									# Hmm -- 2014-01-30, NJM noticed a bug here on a (A,(A,AB)) tree
									# It looks like it should be +1, or everything on UPPASS gets shifted
									# up one state.
									# I have may have changed this because of different context somewhere else
									# (simulations?) where it made sense.
									# 2014-01-30_NJM CONCLUSION: You have to get the ii matches correct, above, first!)
									# Going from "+ 1 + 1" --> "+ 1 + 1"
									# The Lstate_1based_indexes are based on ALL states
								...
								# copied from calc_loglike_sp, 2014-03-20...
								# BUG BUG BUG (maybe)
								# 2014-01-30_NJM
								# ANOTHER BUG SOURCE!! IF "ancstate_matches_TF"
								# IS ALL FALSE, "split_probs_for_this_ancestor"
								# becomes numeric(), which then converts
								# "relprobs_just_after_speciation_UPPASS_Left"
								# and 
								# "relprobs_just_after_speciation_UPPASS_Right"
								# to numeric(0) ALSO!!!!!!
								# ACK!!  This often blasts e.g. the rightmost
								# column (widespread ranges)
								...
								# copied from calc_loglike_sp, 2014-03-20...
								# AVOID ERROR WHEN split_probs_for_this_ancestor is numeric(0)
								if (length(split_probs_for_this_ancestor) > 0)
									{
									# 2014-01-30_NJM BUG FIX: YOU HAVE TO DO A GODDAMN FOR-LOOP TO 
									# *ADD* THE SPLIT PROBS INSTEAD OF WRITING OVER THEM!!!!!!!!!!!
									for (index in 1:length(Lstate_1based_indexes))
										{
										# Buggy version...
...
###										#relprobs_just_after_speciation_UPPASS_Right[Rstate_1based_indexes] = relprobs_just_after_speciation_UPPASS_Right[Rstate_1based_indexes] + split_probs_for_this_ancestor
										...
										# FIXED 2014-03-20
										# Then add to the uppass probs for these branch bottoms
										relprobs_just_after_speciation_UPPASS_Left[Lstate_1based_indexes[index]] = relprobs_just_after_speciation_UPPASS_Left[Lstate_1based_indexes[index]] + split_probs_for_this_ancestor[index]
										relprobs_just_after_speciation_UPPASS_Right[Rstate_1based_indexes[index]] = relprobs_just_after_speciation_UPPASS_Right[Rstate_1based_indexes[index]] + split_probs_for_this_ancestor[index]
										}
									}
						...
						# (check top of left branch)
						#######################################################
						if (!is.null(tmp_fixnode))
							{
							# For multiple fixnodes
							if (length(tmp_fixnode) > 1)
								{
								# Get the matching node
								TF = (left_desc_nodenum == tmp_fixnode)
							...
							if ((length(temporary_fixnode) > 0) && (left_desc_nodenum == temporary_fixnode))
								{
								...
								condprobs_Left_branch_top = condprobs_Left_branch_top * temporary_fixlikes
								}
							} # end if (!is.null(tmp_fixnode))
			

						#######################################################
						# If the states/likelihoods have been fixed at a particular node
						# (check top of right branch)
						#######################################################
						if (!is.null(tmp_fixnode))
							{
							# For multiple fixnodes
							if (length(tmp_fixnode) > 1)
								{
								# Get the matching node
								TF = (right_desc_nodenum == tmp_fixnode)
								temporary_fixnode = tmp_fixnode[TF]
								temporary_fixlikes = c(tmp_fixlikes[TF,])
								} else {
								temporary_fixnode = tmp_fixnode
							...
							if ((length(temporary_fixnode) > 0) && (right_desc_nodenum == temporary_fixnode))
								{
								# If the node is fixed, ignore the calculation for this node, and
								# instead use the fixed likelihoods (i.e., the "known" state) for
								# this node.
								# fix the likelihoods of the (NON-NULL) states
								condprobs_Right_branch_top = condprobs_Right_branch_top * temporary_fixlikes
								}
							} # end if (!is.null(tmp_fixnode))
						} # end if (use_fixnodes_on_uppass)
