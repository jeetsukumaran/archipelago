...
if (length(args) < 4) {
    cat("usage: archipelago-classify.R <NUM-AXES> <TARGET> <TRAINING> [TRAINING [TRAINING [...]]]\n")
    cat("\n")
    cat("where:\n")
    cat("\n")
    cat("<NUM-AXES>     Number of axes to use\n")
    cat("<TARGET>       Path to a CSV file containing the summary \n")
    cat("               statistics of the data to be classified.\n")
    cat("<TRAINING>     Paths to CSV files containing the summary \n")
